...
const seedDatabase = async () => {
  ...
  try {
    ...
    const teams = [
      // Football Teams
      { name: 'France', country_code: 'FR', sport: 'Football', logo_url: 'https://flagcdn.com/fr.svg' },
      { name: 'Germany', country_code: 'DE', sport: 'Football', logo_url: 'https://flagcdn.com/de.svg' },
      { name: 'Brazil', country_code: 'BR', sport: 'Football', logo_url: 'https://flagcdn.com/br.svg' },
      { name: 'Argentina', country_code: 'AR', sport: 'Football', logo_url: 'https://flagcdn.com/ar.svg' },
      { name: 'England', country_code: 'GB', sport: 'Football', logo_url: 'https://flagcdn.com/gb.svg' },
      { name: 'Spain', country_code: 'ES', sport: 'Football', logo_url: 'https://flagcdn.com/es.svg' },
      
      // Cricket Teams
      { name: 'India', country_code: 'IN', sport: 'Cricket', logo_url: 'https://flagcdn.com/in.svg' },
      { name: 'Australia', country_code: 'AU', sport: 'Cricket', logo_url: 'https://flagcdn.com/au.svg' },
      { name: 'Pakistan', country_code: 'PK', sport: 'Cricket', logo_url: 'https://flagcdn.com/pk.svg' },
      { name: 'West Indies', country_code: 'WI', sport: 'Cricket', logo_url: 'https://flagcdn.com/bb.svg' },
      
      // Basketball
      { name: 'USA', country_code: 'US', sport: 'Basketball', logo_url: 'https://flagcdn.com/us.svg' },
      { name: 'China', country_code: 'CN', sport: 'Basketball', logo_url: 'https://flagcdn.com/cn.svg' },
      
      // Tennis
      { name: 'Serbia', country_code: 'RS', sport: 'Tennis', logo_url: 'https://flagcdn.com/rs.svg' },
      { name: 'Russia', country_code: 'RU', sport: 'Tennis', logo_url: 'https://flagcdn.com/ru.svg' }
    ]
    ...
    for (const match of matches) {
      const result = await query(
        `INSERT INTO matches (external_id, sport, home_team_name, away_team_name, start_time, status, home_score, away_score, country_code)
         ...
         RETURNING id`,
        [match.external_id, match.sport, match.home_team_name, match.away_team_name, match.start_time, match.status, match.home_score, match.away_score, match.country_code]
      )
      if (result.length > 0) {
    ...
    if (matchIds.length > 0) {
      ...
      for (const event of events) {
        await query(
          `INSERT INTO match_events (match_id, event_type, minute, player_name, description)
           VALUES ($1, $2, $3, $4, $5)`,
          [event.match_id, event.event_type, event.minute, event.player_name, event.description]
        )
      }
      ...
      const summaries = [
        {
          match_id: matchIds[2],
          summary: 'India vs Australia is heating up! Rohit Sharma was dismissed early, but Virat Kohli is playing brilliantly. India currently at 145/5. Australia needs to capitalize on their bowling advantage.',
          prediction: 'India has a 65% chance of winning based on current form and pitch conditions.',
          confidence_score: 0.72
        },
        {
          match_id: matchIds[3],
          summary: 'England and Spain delivered an entertaining match! England took the lead through Harry Kane early. Spain equalized with a penalty from Busquets, but Saka sealed the victory with a late counter-attack goal.',
          prediction: 'England dominated the match with 62% possession and 14 shots on target.',
      ...
      for (const summary of summaries) {
        await query(
          `INSERT INTO match_ai_summaries (match_id, summary, prediction, confidence_score)
           VALUES ($1, $2, $3, $4)`,
          [summary.match_id, summary.summary, summary.prediction, summary.confidence_score]
        )
      }
