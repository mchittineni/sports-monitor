    ...
    try {
      ...
      const geoJsonLayer = L.geoJSON(geoJsonData, {
        ...
        style: (feature) => {
          ...
          const countryName = codeToCountryName[code] || feature?.properties?.name
    ...
    if (geoJsonLayerRef.current) {
      geoJsonLayerRef.current.eachLayer((layer: L.Layer) => {
        ...
        if (geoJsonLayer.feature) {
          ...
          const countryName = codeToCountryName[code] || geoJsonLayer.feature.properties?.name
          const eventCount = liveEvents.filter((e) => e.country === countryName).length
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className="absolute bottom-4 left-4 bg-black bg-opacity-80 text-white p-3 rounded-lg text-sm z-[999] pointer-events-none">
        ...
        <p className="text-xs mt-2">🔴 Red = 5+ | 🟠 Orange = 2-5 | 🟡 Yellow = 1+ events</p>
      </div>
    </div>
  )
}
